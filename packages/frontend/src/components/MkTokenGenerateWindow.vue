...
const props = withDefaults(defineProps<{
	...
	initialPermissions?: (typeof MisskeyPermissions)[number][] | null;
}>(), {
	title: null,
	information: null,
	initialName: null,
...
const defaultPermissions = MisskeyPermissions.filter(p => !p.startsWith('read:admin') && !p.startsWith('write:admin'));
const adminPermissions = MisskeyPermissions.filter(p => p.startsWith('read:admin') || p.startsWith('write:admin'));

const dialog = shallowRef<InstanceType<typeof MkModalWindow>>();
const name = ref(props.initialName);
const permissionSwitches = ref(<Record<(typeof MisskeyPermissions)[number], boolean>>{});
const permissionSwitchesForAdmin = ref(<Record<(typeof MisskeyPermissions)[number], boolean>>{});
