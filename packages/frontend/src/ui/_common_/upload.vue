		...
		<li v-for="ctx in uploads" :key="ctx.id">
			...
			<div class="top">
				...
				<p class="status">
					...
					<span v-if="ctx.progressValue !== undefined" class="kb">{{ String(Math.floor(ctx.progressValue / 1024)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,') }}<i>KB</i> / {{ctx.progressMax ? String(Math.floor(ctx.progressMax / 1024)).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,'): '?' }}<i>KB</i></span>
					<span v-if="ctx.progressValue !== undefined && ctx.progressMax !== undefined" class="percentage">{{ Math.floor((ctx.progressValue / ctx.progressMax) * 100) }}</span>
				</p>
			</div>
