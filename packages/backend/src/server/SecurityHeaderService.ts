...
export class SecurityHeaderService {
	...
	constructor(
		...
		this.imgSrc = ['\'self\'', 'data:', 'https://xn--931a.moe/assets/error.jpg', 'https://xn--931a.moe/assets/info.jpg', 'https://xn--931a.moe/assets/not-found.jpg', ...(this.config.contentSecurityPolicy.imgSrc)].join(' ');
			...
			switch (request.routeOptions.url) {
				...
				default:
					if (request.routeOptions.url.startsWith('/api/')) {
						...
						enforce = false;
					} else if (request.routeOptions.url.startsWith('/queue/')) {
						// ClientServerService
