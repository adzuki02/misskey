...
export class CacheService implements OnApplicationShutdown {
	...
	public userStrongMutingsCache: RedisKVCache<Set<string>>;
	...
	constructor(
		...
		this.userStrongMutingsCache = new RedisKVCache<Set<string>>(this.redisClient, 'userStrongMutings', {
			lifetime: 1000 * 60 * 30, // 30m
			memoryCacheLifetime: 1000 * 60, // 1m
			fetcher: (key) => this.mutingsRepository.find({ where: { muterId: key, excludeNotification: false }, select: ['muteeId'] }).then(xs => new Set(xs.map(x => x.muteeId))),
			toRedisConverter: (value) => JSON.stringify(Array.from(value)),
			fromRedisConverter: (value) => new Set(JSON.parse(value)),
		});
	...
	public dispose(): void {
		...
		this.userStrongMutingsCache.dispose();
